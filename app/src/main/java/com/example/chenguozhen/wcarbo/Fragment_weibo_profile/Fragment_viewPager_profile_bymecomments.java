...
public class Fragment_viewPager_profile_bymecomments extends BaseListFragment{
    ...
    protected void Create_Content(String token) {
        asyncTask = new BymeCommentAsyncTask(commentsList,getActivity(),BaseAsyncTask.create,token);
        asyncTask.execute(url);
    }

    @Override
    protected void SwipeRefresh_Refresh(String token) {
        asyncTask = new BymeCommentAsyncTask(commentsList,getActivity(),BaseAsyncTask.swipe,token);
        asyncTask.execute(url);
    }
    ...
    protected void ScrollListener_LoadMore(String token) {
        if (max_id != 0) {
            asyncTask = new BymeCommentAsyncTask(commentsList, getActivity(), BaseAsyncTask.scroll, token);
            asyncTask.execute(url);
        }
    }
    ...
    private class BymeCommentAsyncTask extends BaseAsyncTask<Comment>{
        ...
        public BymeCommentAsyncTask(List<Comment> DataList, FragmentActivity fragmentActivity, int type, String token) {
